#!/usr/bin/env python3
"""
H2O (cam4) action segment prompting + optional Qwen inference + CSV/JSONL outputs
- Uses segment ranges (start_act:end_act) from the provided action label file (e.g., action_train.txt)
- Uses raw hand_pose values exactly as stored in hand_pose/*.txt (NO normalization)
- Uses only 6 joints per hand: [0, 4, 8, 12, 16, 20] (wrist + 5 tips)
- Samples exactly N frames per segment, evenly from start_act to end_act
- Builds a joint-wise prompt (temporal coordinate sequence per joint), not a frame-wise prompt
- Includes action_label options in the prompt
- Expects the model to output only the predicted action_label ID
- Writes:
  1) JSONL with prompt + (optional) model outputs
  2) CSV with gt + pred + confidence + rationale + correct
- Shows a progress bar (tqdm) with ETA
"""
...
# 6 joints per hand (OpenPose order): wrist + 5 tips
JOINT_IDXS = [0, 4, 8, 12, 16, 20]
JOINT_NAMES = [
    "Wrist",
    "Thumb tip",
    "Index fingertip",
...
def read_floats(txt_path: Path) -> List[float]:
...
def parse_hand_pose_frame(vals: List[float]) -> Tuple[int, List[List[float]], int, List[List[float]]]:
    """
    Raw H2O hand_pose frame format:
      left:  1 flag + 21*3 = 64 numbers
      right: 1 flag + 21*3 = 64 numbers
      total = 128 numbers

    Returns:
      lf, lj(21x3), rf, rj(21x3)
    """
    if len(vals) < 128:
        raise ValueError(f"Expected 128 floats in hand_pose frame, got {len(vals)}")
    ...
    lj = [left[1 + 3 * i: 1 + 3 * i + 3] for i in range(21)]
    rj = [right[1 + 3 * i: 1 + 3 * i + 3] for i in range(21)]
    return lf, lj, rf, rj
...
def format_xyz_triplet(xyz: List[float]) -> str:
    x, y, z = xyz
    return f"({x:.10f}, {y:.10f}, {z:.10f})"


def format_xyz_sequence(seq: List[List[float]]) -> str:
    return "(" + ", ".join(format_xyz_triplet(xyz) for xyz in seq) + ")"


def format_flag_sequence(flags: List[int]) -> str:
    return "(" + ", ".join(str(int(v)) for v in flags) + ")"


def build_hand_dynamics_block(
    hand_title: str,
    hand_flags: List[int],
    hand_joint_series: Dict[str, List[List[float]]],
) -> str:
    lines = [
        f"## {hand_title}",
        f"{hand_title} valid flag sequence: {format_flag_sequence(hand_flags)}",
        "",
    ]
    for joint_name in JOINT_NAMES:
        lines.append(f"{joint_name}: {format_xyz_sequence(hand_joint_series[joint_name])}")
        lines.append("")
    return "\n".join(lines).strip()
...
def build_prompt(
    left_flags: List[int],
    right_flags: List[int],
    left_joint_series: Dict[str, List[List[float]]],
    right_joint_series: Dict[str, List[List[float]]],
    num_frames: int,
) -> str:
    intro = (
        "You are an action_label detector assistant.\n\n"
        f"You are given the 3D coordinates of 6 hand joints across {num_frames} frames.\n"
        "The 6 joints are:\n"
        "1. Wrist (0)\n"
        "2. Thumb tip (4)\n"
        "3. Index fingertip (8)\n"
        "4. Middle fingertip (12)\n"
        "5. Ring fingertip (16)\n"
        "6. Pinky fingertip (20)\n\n"
        "The input is grouped joint-wise, not frame-wise.\n"
        "For each joint, you will receive a sequence of coordinates collected over multiple frames in temporal order.\n"
        "The values are raw dataset values with no normalization.\n\n"
        "## action_label options\n"
        f"{format_action_options()}\n\n"
    )

    left_block = build_hand_dynamics_block(
        hand_title="Left Hand Dynamics",
        hand_flags=left_flags,
        hand_joint_series=left_joint_series,
    )
    right_block = build_hand_dynamics_block(
        hand_title="Right Hand Dynamics",
        hand_flags=right_flags,
        hand_joint_series=right_joint_series,
    )

    outro = (
        "\n\nYour task is to analyze the temporal motion pattern of these 6 joints and determine the most likely action label.\n\n"
        "Instructions:\n"
        "- Focus on motion over time across all frames.\n"
        "- Compare relative movement of fingertips and wrist.\n"
        "- Use both left-hand and right-hand dynamics when available.\n"
...
def extract_prediction(text: str) -> Tuple[Optional[int], Optional[float], Optional[str], Optional[Dict[str, Any]]]:
    ...
    Preferred format:
      plain integer, e.g. 16

    Also supports fallback JSON:
      {"action_label": 16, "confidence": 0.91, "rationale": "..."}
    """
    text = text.strip()
    if not text:
    ...
    if m:
        try:
            return int(m.group(0)), None, None, {"action_label": int(m.group(0))}
        except Exception:
            pass
...
def sample_frame_indices(start: int, end: int, num_samples: int) -> List[int]:
    ...
    - If the segment is long, frames are evenly spaced.
    - If the segment is short, some indices may repeat.
    """
    if num_samples <= 0:
...
def main():
    ...
    ap.add_argument("--out_jsonl", type=str, default="h2o_prompts_preds.jsonl")
    ap.add_argument("--out_csv", type=str, default="h2o_preds.csv")
    ap.add_argument(
        "--max_prompts",
        type=int,
        default=-1,
    ...
    ap.add_argument("--run_qwen", action="store_true", help="Actually run the model and store predictions.")
    ap.add_argument("--model_id", type=str, default="Qwen/Qwen2.5-7B-Instruct")
    ap.add_argument("--max_new_tokens", type=int, default=32)
    ap.add_argument("--temperature", type=float, default=0.0)

    # Optional guard: skip model call if prompt is too long
    ap.add_argument(
        "--max_input_tokens",
        type=int,
        default=-1,
    ...
    with out_jsonl.open("w", encoding="utf-8") as fjson, out_csv.open("w", newline="", encoding="utf-8") as fcsv:
        ...
        for r in pbar:
            ...
            hp_dir = local_seq / "cam4" / "hand_pose"
            if not hp_dir.exists():
                ...
                writer.writerow({
                    ...
                    "rationale": "",
                    "correct": "",
                    "error": "hand_pose_dir_missing",
                })
                continue

            start, end = r["start_act"], r["end_act"]
            sampled_indices = sample_frame_indices(start, end, num_sampled_frames)
            used_frames = len(sampled_indices)
            pbar.set_postfix_str(f"{r['path']} [{start}:{end}] frames={used_frames}")

            left_flags: List[int] = []
            right_flags: List[int] = []
            left_joint_series: Dict[str, List[List[float]]] = {name: [] for name in JOINT_NAMES}
            right_joint_series: Dict[str, List[List[float]]] = {name: [] for name in JOINT_NAMES}

            try:
                for frame_idx in sampled_indices:
                    fp = hp_dir / f"{frame_idx:06d}.txt"
                    if not fp.exists():
                        raise FileNotFoundError(f"Missing frame file: {fp}")

                    vals = read_floats(fp)
                    lf, lj, rf, rj = parse_hand_pose_frame(vals)

                    left_flags.append(lf)
                    right_flags.append(rf)

                    for joint_name, joint_idx in zip(JOINT_NAMES, JOINT_IDXS):
                        left_joint_series[joint_name].append(lj[joint_idx])
                        right_joint_series[joint_name].append(rj[joint_idx])
                ...
                writer.writerow({
                    ...
                    "rationale": "",
                    "correct": "",
                    "error": f"frame_read_error:{type(e).__name__}",
                })
                continue
            ...
            prompt = build_prompt(
                ...
                left_joint_series=left_joint_series,
                right_joint_series=right_joint_series,
